...
The [MoeLog](https://github.com/Moe-Corp/MoeLog.js) SDK for **Bun**: logs, errors and process
health, at almost no cost inside your application.

> **Alpha.** The protocol may change.

```bash
bun add @moecorp/moelog-bun
```

```ts
import { init, log, error, event } from '@moecorp/moelog-bun'
...
If you use Node, install [`@moecorp/moelog-node`](https://www.npmjs.com/package/@moecorp/moelog-node). Do not install both.
